...
function compressRange(input) {
    const { numbers, notNumbers } = input.reduce((prev, cur) => {
...
function doesntHaveMeasuredTrail(condition) {
    // prettier-ignore
    const conditions = condition instanceof ConditionBuilder ? condition.conditions :
        ...
        at(input) {
            if (typeof input === 'string') {
                try {
                    ...
                    throw wrappedError(err, eatSymbols `Rich Presence Format ${name} got invalid string input: ${err.message}`);
                }
                return `@${name}(${input})`;
            }
            if (input instanceof Condition || input instanceof ConditionBuilder) {
                if (doesntHaveMeasuredTrail(input)) {
                    throw new Error(eatSymbols `Rich Presence Format ${name} got invalid input: must have at least one condition with Measured flag, but got ${input.toString()}`);
                }
...
function makeRichPresenceFormat(params) {
    ...
    if (typeof name !== 'string') {
        throw new Error(eatSymbols `Rich Presence Format expected to have a name as string, but got ${name}`);
    }
...
function makeRichPresenceLookup(params) {
    ...
    let parsedDefaultAt = '';
    if (typeof name !== 'string') {
        throw new Error(eatSymbols `Rich Presence Lookup expected to have a name as string, but got ${name}`);
    }
    const entries = Object.entries(values || {});
    if (entries.length === 0) {
        throw new Error(eatSymbols `Rich Presence Lookup ${name} must define at least one key-value pair`);
    }
    for (const [key, value] of entries) {
        ...
        if (isNumber(key, { isInteger: true, isPositive: true }) === false) {
            throw new Error(eatSymbols `Rich Presence Lookup ${name} got invalid key-value pair ${key}: ${value}, value must be positive integer or "*"`);
        }
    }
    if (defaultAt !== undefined) {
        if (typeof defaultAt === 'string') {
            try {
                ...
                throw wrappedError(err, eatSymbols `Rich Presence Lookup ${name} got invalid defaultAt: ${err.message}`);
            }
            if (doesntHaveMeasuredTrail(conditions[0])) {
                throw new Error(eatSymbols `Rich Presence Lookup ${name} got invalid input: must have at least one condition with Measured flag, but got ${defaultAt}`);
            }
            ...
            if (doesntHaveMeasuredTrail(defaultAt)) {
                throw new Error(eatSymbols `Rich Presence Lookup ${name} got invalid defaultAt: must have at least one condition with Measured flag, but got ${defaultAt}`);
            }
            parsedDefaultAt = `@${name}(${shortenMaybe(defaultAt)})`;
        }
        else {
            throw new Error(eatSymbols `Rich Presence Lookup ${name} defaultAt expected to be a string, Condition or ConditionBuilder, but got ${defaultAt}`);
        }
    }
        ...
        toString(keyFormat = 'dec') {
            ...
            for (const inputKey in finalValues) {
                ...
                if (key !== '*' && keyFormat.startsWith('hex')) {
                    key = key.replace(/\d+/g, num => formatNumberAsHex(Number(num), keyFormat !== 'hex-lowercase'));
                }
        ...
        at: function at(input) {
            ...
            if (typeof input === 'string') {
                try {
                    ...
                    throw wrappedError(err, eatSymbols `Rich Presence Lookup ${name} got error when parsing input: ${err.message}`);
                }
                if (doesntHaveMeasuredTrail(conditions[0])) {
                    throw new Error(eatSymbols `Rich Presence Lookup ${name} got invalid input: must have at least one condition with Measured flag, but got ${input}`);
                }
                return `@${name}(${input})`;
            }
 ...
 *  Playing a good game`
 */
export const RichPresence = (params) => {
    ...
    const mappedDisplays = displays.map((display, i) => {
        ...
        if (Array.isArray(display)) {
            if (display.length !== 2) {
                throw new Error(`Rich Presence displays[${i}] must be either a string or an array with two strings`);
            }
