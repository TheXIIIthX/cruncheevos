...
function getCredentials() {
    ...
    if (!config.Username) {
        throw new Error(`${configFile}: expected Username property as string, but got ${config.Username}`);
    }
    if (!config.Token) {
...
async function fetchRemoteData(opts) {
    ...
    const payload = await fetch(`https://retroachievements.org/dorequest.php?r=achievementsets&t=${token}&u=${username}&g=${gameId}`, {
        headers: {
    ...
    if (payload.Success !== true) {
        throw new Error(`failed to fetch remote data: expected payload.Success to be true, but got ${payload.Success}`);
    }
    return payload;
}
export default async function fetchAssets(opts) {
    const { gameId } = opts;
...
export async function getRemoteData({ gameId, timeout }) {
    const filePath = resolveRACache(`./RACache/Data/${gameId}.json`);
...
export function getSetFromRemoteData(remoteData, setId) {
    if (setId) {
