...
const fs = getFs();
function invalidRichPresenceExport(rich) {
    const valid = typeof rich === 'string' ||
        (typeof rich?.lookup === 'object' &&
            typeof rich?.format === 'object' &&
            Array.isArray(rich?.displayStrings));
    return valid === false;
}
export default async function richSave(inputFilePath, opts) {
    ...
    if (!Object.getOwnPropertyDescriptor(module, 'rich')) {
        log(util.styleText('yellowBright', `set doesn't export a string named 'rich', rich-save aborted`));
        return;
    }
    if (invalidRichPresenceExport(module.rich)) {
        log(util.styleText('yellowBright', `expected set to export a string named 'rich' or object returned by RichPresence, but it exported a ${typeof module.rich} instead, rich-save aborted`));
        return;
    }
