...
let bananaChains = ["/", ...binaryGenerator(1), ...binaryGenerator(2), ...binaryGenerator(3), ...binaryGenerator(4), ...binaryGenerator(5), ...binaryGenerator(6), ...binaryGenerator(7), ...binaryGenerator(8)]
...
for (let i = 0; i < bananaChains.length; i++) {
    set.addAchievement({
        title: "[VOID] Flag test " + i,
        points: 0,
        conditions: $(
            ['', 'Mem', '32bit', 0x004c5b98, '!=', 'Value', '', 0],
            ...bananaPointer(bananaChains[i]),
            ['', 'Mem', '32bit', 0x174, '=', 'Value', '', 0xbf],
        )
    })
}
