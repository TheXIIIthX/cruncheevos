...
import { getRemoteData, getSetFromRemoteData } from './command-fetch.js';
...
function conditionsToJsCodeLines(conditions) {
...
function groupSetToJsCode(groupSet, spaces) {
...
function remoteDataToJSCode(remoteData, { filter, includeUnofficial }) {
    ...
    src += `const set = new AchievementSet({ gameId: ${GameId}, title: ${quoted(remoteData.Title)} })\n\n`;
    for (const ach of Achievements) {
        ...
        try {
            ...
            src += [
                ...
                .join('\n');
            if (lackingPieces) {
                log(util.styleText('yellowBright', `Achievement with ID ${ach.ID} lacks ${lackingPieces}, marked with FIXME`));
            }
        }
        catch (err) {
    ...
    for (const lb of Leaderboards) {
        ...
        if (lackingPieces) {
            log(util.styleText('yellowBright', `Leaderboard with ID ${lb.ID} lacks ${lackingPieces}, marked with FIXME`));
        }
    }
