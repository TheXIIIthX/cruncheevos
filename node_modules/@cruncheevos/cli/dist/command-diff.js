...
function makeAssetDiffReport(opts) {
    ...
    if (titleChanged) {
        pushHeaderLines(['Title', util.styleText('redBright', '- ' + original.title)], ['', util.styleText('greenBright', '+ ' + modified.title)]);
    }
    else {
    ...
    if (titleChanged && descriptionChanged === false) {
        ...
        pushHeaderLines(['Desc.', util.styleText('redBright', '- ' + original.description)], ['', util.styleText('greenBright', '+ ' + modified.description)]);
    }
...
export default async function diff(inputFilePath, opts) {
    ...
    if (inputSetIsEmpty) {
        log(util.styleText('yellowBright', `set doesn't define any achievements or leaderboards, diff aborted`));
    ...
    if (!localData) {
        ...
        log(util.styleText('yellowBright', `local file ${filePath} doesn't exist, will not diff against local file`));
    }
